...
//!
//! Verifies that an ElGamal public key is correctly derived from a secret key.
//! Required to configure a token account for confidential transfers.

use anchor_lang::prelude::*;
use x0_common::error::X0ZkVerifierError;

use crate::state::{ProofContext, ProofType, PROOF_CONTEXT_SEED};
use crate::verification::verify_pubkey_validity_proof;

#[derive(Accounts)]
pub struct VerifyPubkeyValidity<'info> {
    /// The owner of the token account
    #[account(mut)]
    pub owner: Signer<'info>,

    /// The token account being configured
...
pub fn handler(
    ...
    let verified = verify_pubkey_validity_proof(
    ...
    proof_context.proof_type = ProofType::PubkeyValidity;
    ...
    proof_context.elgamal_pubkey = Some(elgamal_pubkey);
    ...
    msg!(
        "PubkeyValidity proof verified: pubkey={:?}, proof_context={}",
        elgamal_pubkey,
        proof_context.key()
    );
