...
\begin{tikzpicture}[
    ...
    external/.style={rectangle, rounded corners=3pt, draw=gray!80, fill=gray!15, 
        minimum width=2.0cm, minimum height=0.8cm, text centered, font=\footnotesize},
    % Core x0 programs
    ...
    library/.style={rectangle, rounded corners=3pt, draw=green!70!black, fill=green!10, 
        minimum width=2.0cm, minimum height=0.8cm, text centered, font=\footnotesize},
    % WASM off-chain module
    wasm/.style={rectangle, rounded corners=3pt, draw=purple!70, fill=purple!10, 
        minimum width=2.0cm, minimum height=0.8cm, text centered, font=\footnotesize},
    % Arrow styles
    arrow/.style={->, >=stealth, thick, color=gray!70},
    lbl/.style={font=\tiny, fill=white, inner sep=1pt}
]
    % --- Row 0: External systems ---
    \node[external] (usdc) {USDC (SPL)};
    \node[external, right=of usdc] (token22) {Token-2022};
    
    % --- Row 1: Core programs (4 nodes, spaced evenly) ---
    \node[program, below=of usdc, xshift=-1.0cm] (wrapper) {x0-wrapper};
    \node[program, right=of wrapper] (token) {x0-token};
    \node[program, right=of token] (guard) {x0-guard};
    \node[program, right=of guard] (zkverifier) {x0-zk-verifier};
    
    % --- Row 2: Core programs (3 nodes, centered under row 1) ---
    \node[program, below=of token, xshift=-1.0cm] (escrow) {x0-escrow};
    \node[program, right=of escrow] (registry) {x0-registry};
    \node[program, right=of registry] (reputation) {x0-reputation};
    
    % --- Row 3: Shared library + WASM ---
    \node[library, below=1.6cm of registry] (common) {x0-common};
    \node[wasm, right=3.0cm of common] (zkproofs) {x0-zk-proofs};
    
    % --- External dependencies (upward arrows) ---
    \draw[arrow] (wrapper) -- (usdc);
    \draw[arrow] (token) -- (token22);
    \draw[arrow] (guard.north) -- ++(0,0.4) -| (token22);
    
    % --- Internal CPI calls (solid arrows) ---
    \draw[arrow] (token) -- (guard) node[lbl, midway, above] {hook};
    \draw[arrow] (wrapper) -- (token);
    \draw[arrow] (escrow) -- (reputation) node[lbl, midway, above] {CPI};
    \draw[arrow] (guard) -- (reputation) node[lbl, midway, right] {CPI};
    \draw[arrow] (registry) -- (reputation);
    \draw[arrow] (token) -- (zkverifier) node[lbl, midway, above] {CPI};
    
    % --- WASM to on-chain (dotted arrow) ---
    \draw[arrow, dotted, color=purple!60] (zkproofs) -- (zkverifier) node[lbl, midway, right] {proofs};
    
    % --- Common library links (dashed arrows) ---
    \draw[arrow, dashed] (escrow) -- (common);
    \draw[arrow, dashed] (registry) -- (common);
    \draw[arrow, dashed] (reputation) -- (common);
    \draw[arrow, dashed] (wrapper.south) -- ++(0,-0.5) -| (common.west);
    \draw[arrow, dashed] (guard.south) -- ++(0,-0.5) -| (common.north);
    \draw[arrow, dashed] (zkverifier.south) -- ++(0,-0.5) -| (common.east);
    
    % --- Legend (well below the diagram) ---
    \matrix[below=2.0cm of common, anchor=center, column sep=6pt, row sep=0pt] {
        \node[external, minimum width=1.2cm, minimum height=0.45cm, font=\tiny] {External}; &
        \node[program, minimum width=1.2cm, minimum height=0.45cm, font=\tiny] {Program}; &
        \node[library, minimum width=1.2cm, minimum height=0.45cm, font=\tiny] {Library}; &
        \node[wasm, minimum width=1.2cm, minimum height=0.45cm, font=\tiny] {WASM}; &
        \node[font=\tiny] (la) {}; \draw[arrow] (la.west) -- ++(0.7,0) node[right, font=\tiny] {CPI}; &
        \node[font=\tiny] (lb) {}; \draw[arrow, dashed] (lb.west) -- ++(0.7,0) node[right, font=\tiny] {uses}; \\
    };
\end{tikzpicture}
\end{center}

\begin{center}
\small\textit{Figure 1: x0 Protocol Architecture. Gray nodes are external Solana programs. Blue nodes are x0 on-chain programs. Green is the shared library. Purple is the off-chain WASM module for client-side proof generation.}
\end{center}
...
\begin{proof}
Follows from the knowledge-soundness of Groth16 under the $q$-type knowledge-of-exponent assumption in bilinear groups. The verifier program re-derives the verification via \texttt{solana-zk-token-sdk}'s \texttt{verify()} method, which checks the Groth16 pairing equation. \qed
\end{proof}
...
\begin{itemize}
    \item Wormhole integration for cross-chain messages and proof relay
    \item Unified reputation across chains, anchored to Solana as the settlement layer
    \item Cross-chain escrow settlement with atomic swap guarantees
\end{itemize}
...
\hline
\end{tabular}
\caption{Protocol Error Codes (representative subset; 7 enums, 60+ total codes). Full reference in source: \texttt{x0-common/src/error.rs}}
\end{table}
