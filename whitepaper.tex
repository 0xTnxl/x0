...
\subsection{Confidential Transfer Warning}

\textbf{Important}: The SDK's \texttt{ConfidentialClient} module uses \textit{placeholder} ZK proof generation and is marked as \texttt{@experimental}. It should not be used in production environments.

\begin{lstlisting}[language=JavaScript,caption=Confidential Transfer Limitations]
// WARNING: EXPERIMENTAL - Uses placeholder ZK proofs
// Do NOT use confidential transfers in production without
// implementing real Groth16 proof generation via solana-zk-token-sdk

import { ConfidentialClient } from '@x0-protocol/sdk';

// This client will emit console warnings at runtime
const confidentialClient = new ConfidentialClient(connection, wallet);
\end{lstlisting}

\textbf{Production Requirements}: Before enabling confidential transfers on mainnet:
\begin{enumerate}
    \item Integrate with Solana's ZK Token SDK for authentic Groth16 proof construction
    \item Replace placeholder proof functions (\texttt{generatePubkeyValidityProof}, \texttt{generateWithdrawProof}, \texttt{generateZeroBalanceProof}) with real ZK proving system
    \item Use \texttt{maxPendingCredits} parameter matching protocol constant (65536) instead of u64::MAX
    \item Implement proper ElGamal encryption and range proof verification
    \item Test thoroughly on devnet with real ZK proofs before mainnet deployment
\end{enumerate}

\textbf{Note}: The on-chain validation (x0-guard) \textit{does} perform proper ZK proof verification. The limitation is only in the TypeScript SDK's client-side proof generation, which currently returns synthetic test data.
